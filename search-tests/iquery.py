...
def register_filter_factory(
    django_field_class, *, shortcut_name=MISSING, factory=MISSING, **kwargs
):
...
def boolean__query_operator_to_q_operator(op):
    if op != "=":
        raise QueryException(
            f'Invalid operator "{op}" for boolean filter. The only valid operator is "=".'
        )
    return "exact"
...
def choice_queryset_value_to_q(filter, op, value_string_or_f, **_):
    if op != "=":
        raise QueryException(
            f'Invalid operator "{op}" for filter "{filter._name}"'
        )
    if filter.attr is None:
        return Q()
    if (
        isinstance(value_string_or_f, str)
        and value_string_or_f.lower() == "null"
    ):
    ...
    try:
        ...
        for search_field in filter.search_fields:
            try:
                instance = filter.choices.get(
                    **{search_field: str(value_string_or_f)}
                )
                break
        ...
        raise QueryException(
            f'Found more than one object for name "{value_string_or_f}"'
        )
    if instance is None:
        return None
...
class Filter(Part):
        ...
        if is_str and value_string_or_f.lower() == "null":
            ...
            r = Q(
                **{
                    filter.attr
                    + "__"
                    + filter.query_operator_to_q_operator(op): value_string_or_f
                }
            )
        if negated:
            return ~r
    ...
    @classmethod
    def from_model(
        cls, model, model_field_name=None, model_field=None, **kwargs
    ):
...
class Query(Part):
    ...
    @with_defaults(
        ...
        form__attrs={
            "data-iommi-errors": lambda query,
            **_: query.endpoints.errors.iommi_path
        },
    ...
    def _on_refine_done_form(self):
        ...
        freetext_search_config = self.iommi_namespace.form.get(
            "fields", {}
        ).get(FREETEXT_SEARCH_NAME, {})
        if freetext_search_config is not None:
            declared_fields[FREETEXT_SEARCH_NAME] = setdefaults_path(
                ...
                required=False,
                include=lambda query, **_: any(
                    filter.freetext for filter in values(query.filters)
                ),
                help__include=False,
            )

        for name, filter in items(self.iommi_namespace.filters):
            _orig_include = getattr_path(
                filter, "field__include", not filter.freetext
            )
        ...
        for name, field in items(declared_fields):
            ...
            if (
                name in declared_filters
                and name not in self.iommi_namespace.filters
            ):
    ...
    def on_bind(self) -> None:
        ...
        self.query_advanced_value = (
            request.GET.get(self.get_advanced_query_param(), "").strip()
            if request
            else ""
        )
        ...
        self.filter_name_by_query_name = {
            x.query_name: name for name, x in items(self.filters)
        }
    ...
    def _create_grammar(self):
        ...
        binary_operator_statement = (
            identifier + binary_op + value_string
        ).setParseAction(self._binary_op_to_q)
        unary_operator_statement = (
            identifier | (Char("!") + identifier)
        ).setParseAction(self._unary_op_to_q)
    ...
    def _unary_op_to_q(self, token):
        if len(token) == 1:
            ...
            if (
                op != "!"
            ):  # pragma: no cover. You can't actually get here because you'll get a syntax error earlier
                raise QueryException(
        ...
        if filter:
            ...
            result = filter.invoke_callback(
                filter.value_to_q, op="=", value_string_or_f=value
            )
            return result
        raise QueryException(
    ...
    def _binary_op_to_q(self, token):
        ...
        try:
            result = filter.invoke_callback(
                filter.value_to_q, op=op, value_string_or_f=value_string_or_f
            )
        except ValidationError as e:
            raise QueryException(f"{e.message}")
        if result is None:
            raise QueryException(
                f'Unknown value "{value_string_or_f}" for filter "{query_name}"'
            )
        return result
            ...
            [
                Q(
                    **{
                        filter.attr
                        + "__"
                        + filter.query_operator_to_q_operator(":"): token
                    }
                )
    ...
    def get_query_string(self):
        ...
        if self.query_advanced_value:
            ...
            def expr(field, is_list, value):
                ...
                return build_query_expression(
                    filter=self.filters[field._name], value=value
                )
    ...
    def _lowercase_filters(self):
        if not hasattr(self, "_lowercase_filters_cache"):
            self._lowercase_filters_cache = {
                k.lower(): v for k, v in items(self.filters)
            }
    ...
    def _from_model(cls, *, rows=None, model=None, include=None, exclude=None):
        ...
        assert (
            model is not None or rows is not None
        ), "auto__model or auto__rows must be specified"
        filters = cls.filters_from_model(
            model=model, include=include, exclude=exclude
        )
